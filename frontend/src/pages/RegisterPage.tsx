...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.adminName.trim()) { setError('Your full name is required.'); return; }
    if (!form.email.trim())    { setError('Work email is required.'); return; }
    if (form.password.length < 8) { setError('Password must be at least 8 characters.'); return; }

    setLoading(true);
    try {
      await createOrg(form.name.trim(), form.adminName.trim(), form.email.trim(), form.password);
      setSuccess(true);
      setTimeout(() => navigate('/login'), 2500);
    } catch (err: any) {
      const msg = err?.response?.data?.message ?? '';
      if (msg.toLowerCase().includes('already exists') || msg.toLowerCase().includes('dup')) {
  ...
  return (
    <div className="dark-auth-wrap">
      <div className="dark-auth-card" style={{ maxWidth: 980 }}>

        {/* Image panel on left */}
        <div className="dark-auth-right" style={{ order: -1, borderRadius: '18px 0 0 18px' }}>
          <img src="https://images.pexels.com/photos/3184291/pexels-photo-3184291.jpeg?auto=compress&cs=tinysrgb&w=900" alt="Team" />
        ...
        <div className="dark-auth-left" onMouseMove={onMouseMove}
          ...
          <div className="dark-auth-form">
            ...
            <div>
              <h1 style={{ color: 'var(--auth-heading)', fontSize: 24, fontWeight: 900, marginBottom: 4 }}>Create your workspace</h1>
              <p style={{ color: 'var(--auth-muted)', fontSize: 13 }}>Sign up instantly with a provider or fill in your details below.</p>
            </div>

            {/* OAuth Buttons */}
            <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginBottom: 16 }}>
              {OAUTH_PROVIDERS.map(p => (
                <a key={p.label} href={p.href} title={`Sign up with ${p.label}`}
                  style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <span style={{ color: 'var(--auth-muted)', fontSize: 12, whiteSpace: 'nowrap' }}>or register with email</span>
              <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.08)' }} />
            </div>

            {error   && <div className="dark-error">{error}</div>}
            {success && (
              <div style={{ background: 'rgba(34,197,94,.12)', border: '1px solid rgba(34,197,94,.3)', color: '#86efac', borderRadius: 8, padding: '10px 14px', fontSize: 13 }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 11 }}>
                <DarkInput placeholder="Organisation name *" value={form.name} onChange={set('name')} />
                <DarkInput placeholder="Your full name *" value={form.adminName} onChange={set('adminName')} />
              </div>
